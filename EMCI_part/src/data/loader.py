...
class DataLoader:
    ...
    Sources:
    - Yahoo Finance: EEM (MSCI EM), DXY, VIX, Treasury yields, commodities, FX rates
    - FRED: Fed Funds Rate, Treasury spreads (optional)
    """
    ...
    def fetch_target(
        ...
        """
        Fetch target asset (EEM — MSCI Emerging Markets ETF) data
        
        Returns:
            DataFrame with target OHLCV data
        """
        start = start_date or self.config.start_date
        end = end_date or self.config.end_date
        
        ticker = self.config.target_ticker
        ...
        if not df.empty:
            ...
            logger.error(f"✗ Failed to fetch {ticker} data")
